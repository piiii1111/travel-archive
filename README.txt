...
- 詳情 Hero 不再顯示編輯與刪除；兩項操作整合至旅程資訊標題列。
- 「編輯旅程資訊」改為「編輯旅途」。
- 手機版 Journey 操作列貼齊 Modal 底部，不再留下大片空白。
- 地標完整名稱找不到時，會再嘗試城市前綴或「溫泉」等尾詞移除後的核心名稱。
- 本版本不需要執行新的 SQL。
...
v1.6.3 修正：
- 修改 Journey 預設匯率後，同步更新該 Journey 所有已保存的非 TWD 費用；TWD 匯率固定為 1。
- Expense Modal 改為只能使用右上角關閉鈕或取消按鈕關閉，桌機拖曳選取文字不再誤關閉。
- 所有直接新增 Master Data 的按鈕，保存成功後立即更新前端選項與管理畫面，不需重新整理。
- 旅程資訊與費用頁統一內容寬度、頁首位置與手機版操作按鈕外觀。
- 已輸入與未完成筆數改為可點選的費用篩選按鈕；最後一筆缺漏修正後自動恢復顯示全部資料。
- 本次不需要新增或執行 Supabase SQL，只需更新網站檔案。

v1.6.4 修正：
- 修改 Journey 預設匯率時，先顯示受影響費用筆數並要求確認；只同步仍使用舊預設匯率的費用，自訂匯率不變。
- 費用可自由選擇日期並自動判斷旅行前／中／後及對應 Day；選擇 Day 時也會帶入該日日期。
- 未完成費用為 0 時，篩選畫面顯示「全部都已填寫完整」。
- Journey 標題、Meta 與總心得區塊可直接點擊開啟旅程編輯。
- 首頁移除載入前的靜態範例卡片，刪除旅程後也立即移除前端快取，避免重新整理時短暫出現舊卡片。
- 桌機首頁 Journey 區塊固定排列於 Intro 下方，不再受左側地圖高度影響。
- 本次不需要新增或執行 Supabase SQL，只需更新網站檔案。

v1.6.5 修正：
- 城市名稱統一忽略「市／縣」尾碼，福岡、福岡縣與小倉、小倉市皆會依同一筆城市處理。
- 「全部都已填寫完整」沿用一般空狀態的字級與字重。
- 修改 Journey 開始／結束日期後，既有 Day 會從新開始日依序更新；day-date、dayEditDate 與 Expense Day 日期同步重新載入。
- 超出 Journey 結束日的額外 Day 仍可在確認後新增，保留轉機與延伸行程用途。
- 桌機首頁改為獨立的右側欄，Journey section-head 固定緊接在 Intro 下方。
- 拖曳 Day 時改用兩階段日期更新，避免日期唯一值衝突；費用所屬 Day 會依新順序同步顯示。
- 本次不需要新增或執行 Supabase SQL，只需更新網站檔案。

v1.6.6 修正：
- 「九州動物園」定位改為九州自然動物公園 African Safari（大分縣宇佐市），避免誤判成福岡的法務局。
- 費用期間篩選改為「全部／出發前／Day 1…／旅遊後」，Day 選項依目前 Journey 實際資料產生。
- 期間篩選移至類別統計右上角；總花費、筆數、缺漏、類別統計、付款來源與費用明細會同步套用篩選。
- 點選旅遊總心得會定位到 Journey Modal 的心得欄位；日期 Meta 定位到開始日期；編輯旅途按鈕固定回到 Modal 頂端。
- 缺漏提醒卡只在目前篩選範圍內真的有未填完整的費用時顯示；全部完整時自動隱藏。
- 本次不需要新增或執行 Supabase SQL，只需更新網站檔案。

v1.6.7 修正：
- 修改旅程主要幣別時，只同步仍使用舊主要幣別的費用；其他單筆自訂幣別保留，儲存前會先顯示影響筆數。
- 費用群組固定依「出發前／Day 1／Day 2…／旅遊後」排列。
- 刪除、拖曳、編輯 Day 或修改旅程日期後，Day 日期及其綁定的費用日期、階段、Day 編號同步更新。
- 所有頁面右下角新增回到頂端按鈕；Modal 開啟時會回到該 Modal 頂端。
- 本次不需要新增或執行 Supabase SQL，只需更新網站檔案。

v1.6.8 修正：
- 代表地點標題新增可展開／收起的「？」定位說明；台灣完整地址會自動移除郵遞區號並拆分縣市、鄉鎮區與道路後重試。
- 首頁「地圖」文案改為「旅圖」，旅程內頁返回按鈕改為「回到首頁」。
- 新增或修改節點時間後自動依時間插入正確位置；手動拖曳仍維持交換時段的既有規則。
- 修正 iPhone 瀏覽器將 Canvas WebP 回傳成 PNG MIME，導致 Supabase 拒絕上傳的問題。
- 旅程代表照片右上角新增移除按鈕，儲存後可清除已上傳照片。
- 本次不需要新增或執行 Supabase SQL，只需更新網站檔案。
