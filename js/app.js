const journeys = window.prototypeJourneys || [];
let map;
...
function renderTimeline(){
  ...
  root.innerHTML=Object.keys(groups).sort((a,b)=>b-a).map(year=>{
    ...
    return `<section class="archive-year-group"><div class="archive-year-label"><span></span><b>${year}</b></div><div class="archive-year-trips">${trips.map(j=>`<button class="archive-trip-card" type="button" onclick="openDetail()"><img src="${j.photo}" alt="${j.title}"><span class="archive-trip-body"><b>${j.title}</b><small>${j.date}</small><small>${j.country} · ${j.cities.join('、')}</small></span></button>`).join('')}</div></section>`
  }).join('');
}
function initMap(){
  ...
  const pts=[[25.1676,121.445],[21.946,120.797],[35.1815,136.9066],[35.6762,139.6503]];
  journeys.forEach((j,i)=>{const icon=L.divIcon({className:'photo-pin-wrap',html:`<button class="photo-pin"><img src="${j.photo}" alt="${j.title}"></button>`,iconSize:[48,48],iconAnchor:[24,24]});L.marker(pts[i],{icon}).addTo(map).bindPopup(`<button class="map-trip-popup" onclick="openDetail()"><img src="${j.photo}"><span><b>${j.title}</b><small>${j.date}</small></span></button>`)});
}
...
function openDetail(){document.getElementById('homeView').classList.add('hidden');document.getElementById('detailView').classList.add('active');window.scrollTo(0,0)}
function closeDetail(){document.getElementById('detailView').classList.remove('active');document.getElementById('homeView').classList.remove('hidden');window.scrollTo(0,0);if(map)setTimeout(()=>map.invalidateSize(),50)}
...
function toggleDayMenu(btn){btn.nextElementSibling?.classList.toggle('show')}
